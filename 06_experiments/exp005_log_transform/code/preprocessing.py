"""
exp005_log_transform 専用の前処理コード

exp004をベースに以下の変更:
- old_building_flag を削除（特徴量重要度88位/89、building_ageと冗長）
...
# =============================================
# 数値特徴量（exp004から継承、old_building_flag削除）
# =============================================
...
# exp005で使用する築年数関連特徴量（old_building_flag削除）
AGE_NUMERIC_FEATURES = [
    'building_age',          # 築年数（8位）
    'building_age_bin',      # 築年数5年単位カテゴリ（16位）
    # 'old_building_flag',   # 削除：88位/89、building_ageと冗長
    'old_and_large_flag',    # 築35年以上 & 80㎡以上（49位）
    'old_and_rural_flag',    # 築35年以上 & 地方（65位）
]
...
def add_age_features(df: pl.DataFrame) -> pl.DataFrame:
    """築年数関連特徴量を追加（exp005: old_building_flag削除）"""
    # Step 1: 基本特徴量を追加
    df = df.with_columns([
        ...
        # old_building_flag は削除（exp005では使用しない）
...
def preprocess_for_training(
    ...
    """
    print("=" * 60)
    print("前処理開始（exp005: log_transform）")
    print("=" * 60)
    ...
    print("  → 築年数関連特徴量追加（4個、old_building_flag削除）")
    ...
    print(f"  → 特徴量数: {len(ALL_FEATURES)}個（exp004から1個削減）")
