"""
exp005_log_transform training script

Based on exp004 with the following changes:
1. Log transform for target variable (log1p)
2. learning_rate: 0.01 -> 0.03
3. n_estimators: 10,000 -> 15,000
4. Remove old_building_flag

Target: CV MAPE < 17.0% (exp004: 18.60%, improvement > 1.5pt)
"""
...
def train_exp005():
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(cv_splits):
            ...
            # Train (sklearn API)
            # Note: Metrics during training are reference only for log-transformed target
            model.fit(
                X_tr, y_tr,
        ...
        print("=" * 60)
        print(f"  Baseline (exp004): 18.60%")
        print(f"  This result:       {np.mean(cv_scores):.2f}%")
        improvement = 18.60 - np.mean(cv_scores)
        if improvement > 0:
