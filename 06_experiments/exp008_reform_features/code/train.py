"""
exp008_reform_features training script

Based on exp007 with the following changes:
1. Add reform SVD features (wet + interior combined, 7 dims)
2. Add reform year features (years_since_wet_reform, years_since_interior_reform)
3. Add postal code TE (post1_te, post_full_te with fallback)
4. Fix reference year: use building_create_date instead of 2024

Target: CV MAPE < 13.5% (exp007: 13.62%, improvement > 0.1pt)
"""
...
def train_exp008():
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        print(f"  - Reform SVD: {REFORM_SVD_DIM} (NEW)")
        print(f"  - Reform year features: 2 (NEW)")
        print(f"  - Postal code TE: 2 (NEW)")
        ...
        mlflow.log_param("building_tag_svd_dim", BUILDING_TAG_SVD_DIM)
        mlflow.log_param("unit_tag_svd_dim", UNIT_TAG_SVD_DIM)

        # exp008-specific parameters (NEW)
        mlflow.log_param("reform_svd_dim", REFORM_SVD_DIM)
        mlflow.log_param("post_full_min_count", POST_FULL_MIN_COUNT)
        ...
        for i, idx in enumerate(sorted_indices[:20]):
            ...
            # Mark new features
            if feat_name.startswith('reform_svd_') or feat_name in ['years_since_wet_reform', 'years_since_interior_reform']:
                ...
                marker = "★"
            elif feat_name.startswith('tfidf_') or feat_name.startswith('geo_pca_'):
                marker = "☆"
        ...
        # Display new feature importance (exp008 - reform & postal)
        print("\n  New feature importance (exp008 - reform & postal):")
        new_features = [f for f in ALL_FEATURES if
                        f.startswith('reform_svd_') or
                        f in ['years_since_wet_reform', 'years_since_interior_reform',
                              'post1_te', 'post_full_te']]
