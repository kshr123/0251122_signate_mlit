"""
exp011_loss_function training script

損失関数の比較実験。低価格帯の予測精度改善を目指す。

実験内容:
- Phase 1: huber, quantile, sample_weight（パラメータ変更のみ）
- Phase 2: mape, focal（カスタム目的関数）

使用方法:
    # Phase 1: Built-in objectives
    python train.py --objective huber --test
    python train.py --objective quantile --alpha 0.3
    python train.py --objective sample_weight --weight-transform inverse

    # Phase 2: Custom objectives
    python train.py --objective mape --hessian-const 1.0
    python train.py --objective focal --gamma 2.0

    # Load preprocessed features（2回目以降）
    python train.py --objective huber --features-dir outputs/run_huber_xxx/
"""
...
from constants import EXP010_FEATURE_COLUMNS, REMOVE_FEATURES
...
def train_exp011(
    ...
    """Train exp011 loss_function experiment
    ...
    with mlflow_context:
        if not test_mode:
            ...
            mlflow.set_tag("base_experiment", exp_config.get("base", "exp010"))
        ...
        if features_dir:
            ...
            if not test_mode:
                print("\n" + "=" * 60)
                print("exp011 Preprocessing (exp010 base)")
                print("=" * 60)
        ...
        sample_weight = None
        custom_objective = None  # BaseObjective instance or None
        ...
        elif objective_type == "quantile":
            lgb_params["objective"] = "quantile"
            lgb_params["alpha"] = loss_config.get("alpha", 0.5)
            if not test_mode:
                print(f"  Using Quantile loss (alpha={lgb_params['alpha']})")
        ...
        elif objective_type == "mape":
            # Custom MAPE objective
            hessian_const = loss_config.get("hessian_const", 1.0)
            custom_objective = MAPEObjective(hessian_const=hessian_const)
            if not test_mode:
                print(f"  Using custom MAPE objective (hessian_const={hessian_const})")
        elif objective_type == "focal":
            # Custom Focal objective
            gamma = loss_config.get("gamma", 2.0)
            custom_objective = FocalObjective(gamma=gamma)
            if not test_mode:
                print(f"  Using custom Focal objective (gamma={gamma})")
        else:
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(cv_splits):
            ...
            # Create model
            # For custom objectives (mape, focal), pass the objective function
            model_params = lgb_params.copy()
            if custom_objective is not None:
                model_params["objective"] = custom_objective
                # Custom objectives need 'none' metric (LightGBM will use objective for eval)
            ...
            # Add custom eval metric for early stopping (Phase 2 custom objectives)
            if custom_objective is not None:
                fit_params["eval_metric"] = custom_objective.eval_metric
        ...
        print(f"  Baseline (exp010): 12.48%")
        print(f"  This result:       {np.mean(cv_scores):.2f}%")
        improvement = 12.48 - np.mean(cv_scores)
        if improvement > 0:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="exp011 loss_function training",
...
Examples:
  ...
  # Phase 2: Custom objectives
  python train.py --objective mape
  python train.py --objective focal --gamma 2.0
  ...
  python train.py --objective huber --features-dir outputs/run_huber_xxx/
    ...
    parser.add_argument("--objective", type=str, required=True,
                        choices=["mse", "huber", "quantile", "sample_weight", "mape", "focal"],
                        help="Objective type: mse, huber, quantile, sample_weight, mape, focal")
    ...
    parser.add_argument("--alpha", type=float, default=None,
                        help="Quantile alpha (default: 0.5)")
    ...
    parser.add_argument("--hessian-const", type=float, default=None,
                        help="MAPE hessian constant (default: 1.0, range: 0.1-2.0)")
    ...
    if args.alpha is not None:
        cli_overrides["alpha"] = args.alpha
    ...
    if args.hessian_const is not None:
        cli_overrides["hessian_const"] = args.hessian_const
