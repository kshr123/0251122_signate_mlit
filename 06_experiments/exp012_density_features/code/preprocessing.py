"""
exp012_density_features 前処理モジュール

exp011をベースに、密度特徴量を追加した前処理。

前処理内容:
1. 基本前処理（年変換、築年数、アクセス時間等）
2. 地価公示データ結合（年度別に最近傍探索）
3. 特徴量パイプライン実行（密度特徴量追加）

使用例:
    X_train, X_test, y_train, pipeline = preprocess_for_training(train, test, cv_splits, config)
