"""
exp006_route_features 専用の前処理コード

exp005をベースに以下の変更:
1. 交通アクセス特徴量の追加（路線名TE, TF-IDF, LE, CE, 時間）
2. 位置情報のPCA次元圧縮（4次元→2次元）

特徴量数: 88 → 123（TF-IDF 20次元の場合）
"""
...
def preprocess_for_training(
    ...
    """
    print("=" * 60)
    print("前処理開始（exp006: route_features）")
    print("=" * 60)
    ...
    print(f"  → 特徴量数: {len(ALL_FEATURES)}個（exp005: 88 → {len(ALL_FEATURES)}）")

    # 新規特徴量の統計を表示
    print("\n[アクセス時間特徴量の統計]")
    for col in ACCESS_NUMERIC_FEATURES:
