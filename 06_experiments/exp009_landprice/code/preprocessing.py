"""
exp009_landprice 専用の前処理コード

exp008をベースに以下の変更:
1. 地価公示データの結合（最近傍探索）
2. 地価公示特徴量の追加（39次元）
   - 土地属性PCA: 1次元
   - 道路状況SVD: 13次元 + 数値1次元 = 14次元
   - 周辺環境LE: 1次元
   - 価格: 3次元
   - 価格時系列: 2次元
   - カテゴリ別地価比率: 9カテゴリ × 2 = 18次元
...
- 郵便番号TE（2次元）
- 面積地域平均比率（6次元）
- 基準年（building_create_date）

特徴量数: 181 → 220（+39地価公示次元）
"""
...
from landprice_features import (
    ...
    LandpriceShapePCATransformer,
    LandpriceRoadSVDTransformer,
    LandpriceCurrentUseLETransformer,
...
# 地価公示特徴量カラム（39次元: 21基本 + 18カテゴリ比率）
LANDPRICE_FEATURE_COLUMNS = get_landprice_feature_columns(include_ratio=True)
...
def extract_reference_year(df: pl.DataFrame) -> pl.DataFrame:
    """snapshot_create_dateから基準年を抽出（★exp009修正: building_create_date→snapshot_create_date）"""
    return df.with_columns([
...
def add_age_features(df: pl.DataFrame) -> pl.DataFrame:
    """築年数関連特徴量を追加（★exp008: 基準年を動的に、★exp009: 未来物件対応）"""
    # Step 1: 生の築年数を計算
    raw_building_age = pl.col("reference_year") - pl.col("year_built")

    # Step 2: 基本特徴量を追加（★exp009: 未来物件フラグ追加、築年数は0でクリップ）
    df = df.with_columns([
        # 未来物件フラグ（★exp009追加: 築年数がマイナス=データ入力ミスor新築予定）
        (raw_building_age < 0).cast(pl.Int64).alias("is_future_building"),

        # 築年数（★exp009修正: マイナスの場合は0にクリップ）
...
class TfidfTransformer:
    ...
    def fit(self, df: pl.DataFrame) -> "TfidfTransformer":
        """学習データでfit"""
        texts = self._create_text(df)
        self.vectorizer.fit(texts)
        self.feature_names = [f'tfidf_{i}' for i in range(self.vectorizer.max_features)]
        return self
...
class LabelEncodingBlock:
    ...
    def fit(self, df: pl.DataFrame) -> "LabelEncodingBlock":
        """学習データでfit"""
...
class GeoPCATransformer:
    ...
    def fit(self, df: pl.DataFrame) -> "GeoPCATransformer":
        """学習データでfit"""
        geo_data = df.select(GEO_COLUMNS).to_numpy()

        # 欠損値を中央値で埋める
        self._train_median = np.nanmedian(geo_data, axis=0)
        geo_data = np.nan_to_num(geo_data, nan=self._train_median)

        self.scaler.fit(geo_data)
        geo_scaled = self.scaler.transform(geo_data)
        self.pca.fit(geo_scaled)
        return self
...
class TagSVDTransformer:
    ...
    def fit(self, df: pl.DataFrame) -> "TagSVDTransformer":
        """学習データでfit"""
...
class ReformSVDTransformer:
    ...
    def fit(self, df: pl.DataFrame) -> "ReformSVDTransformer":
        """学習データでfit"""
        tag_lists = self._parse_tags(df)
...
def get_feature_names(tfidf_dim: int = TFIDF_MAX_FEATURES) -> List[str]:
    ...
    # ★exp009追加: 地価公示特徴量（21次元）
    features.extend(LANDPRICE_FEATURE_COLUMNS)
...
def preprocess_for_training(
    ...
    """
    print("=" * 60)
    print("前処理開始（exp009: landprice_features）")
    print("=" * 60)
    ...
    # ★exp008追加: 郵便番号TE
    print("\n[4/14] 郵便番号TE（post1_te, post_full_te）★exp008")
    ...
    # ★exp008追加: リフォームSVD特徴量
    print("\n[11/14] リフォームSVD（reform_wet_area + reform_interior統合）★exp008")
    ...
    # ★exp009追加: 地価公示データ結合＆特徴量生成
    print("\n[13/15] 地価公示データ結合＆特徴量生成 ★exp009")
    print(f"  → 地価公示データパス: {LANDPRICE_BASE_PATH}")

    # train/testに地価公示データを結合（年度別最近傍探索）
    ...
    # 地価公示特徴量を生成（transformerはtrain時にfit）
    train_with_lp_features, lp_transformers = create_landprice_features(
        train_with_lp, is_train=True
    )
    test_with_lp_features, _ = create_landprice_features(
        test_with_lp,
        shape_pca=lp_transformers["shape_pca"],
        road_svd=lp_transformers["road_svd"],
        current_use_le=lp_transformers["current_use_le"],
        is_train=False
    )

    # 地価公示基本特徴量のみを抽出（21次元）
    lp_base_columns = get_landprice_feature_columns(include_ratio=False)
    train_landprice_base = train_with_lp_features.select(lp_base_columns)
    test_landprice_base = test_with_lp_features.select(lp_base_columns)
    print(f"  → {len(lp_base_columns)}次元の基本地価公示特徴量")

    # ★exp009追加: カテゴリ別地価比率特徴量（18次元）
    print("\n[14/15] カテゴリ別地価比率特徴量 ★exp009")
    lp_ratio_transformer = LandpriceCategoryRatioTransformer()

    # train/testそれぞれ独立に計算（リーケージなし）
    # fit/transformパターンに従う（インターフェース統一のため）
    print("  [train]")
    lp_ratio_transformer.fit()
    train_lp_ratio = lp_ratio_transformer.transform(train_with_lp_features)
    print("  [test]")
    test_lp_ratio = lp_ratio_transformer.transform(test_with_lp_features)
    print(f"  → {train_lp_ratio.shape[1]}次元のカテゴリ別地価比率特徴量")

    # 地価公示特徴量を結合（21 + 18 = 39次元）
    train_landprice = pl.concat([train_landprice_base, train_lp_ratio], how="horizontal")
    test_landprice = pl.concat([test_landprice_base, test_lp_ratio], how="horizontal")
    print(f"  → 合計: {train_landprice.shape[1]}次元の地価公示特徴量")

    lp_null_count = sum([train_landprice[col].null_count() for col in train_landprice.columns])
    lp_total_cells = len(train_landprice) * train_landprice.shape[1]
    print(f"  → 欠損率: {lp_null_count / lp_total_cells:.1%}")
    ...
    print(f"  → 特徴量数: {len(ALL_FEATURES)}個（exp008: 175 → {len(ALL_FEATURES)}）")
    ...
    # ★exp009追加特徴量の統計
    print("\n[exp009追加特徴量の統計（地価公示）]")
    lp_core_cols = ['lp_price', 'lp_change_rate', 'lp_nearest_dist', 'lp_shape_pca', 'lp_current_use_le']
    for col in lp_core_cols:
        if col in X_train.columns:
            non_null = X_train[col].drop_nulls()
            mean_val = non_null.mean() if len(non_null) > 0 else float('nan')
            null_rate = (len(X_train) - len(non_null)) / len(X_train) * 100
    ...
    # 道路SVD特徴量（代表1個のみ）
    if 'lp_road_svd_0' in X_train.columns:
        mean_val = X_train['lp_road_svd_0'].mean()
        std_val = X_train['lp_road_svd_0'].std()
        print(f"  lp_road_svd_*: 次元=13, lp_road_svd_0平均={mean_val:.4f}, 標準偏差={std_val:.4f}")

    # 価格時系列特徴量
    for col in ['lp_ratio_1to3', 'lp_ratio_3to5']:
        if col in X_train.columns:
            non_null = X_train[col].drop_nulls()
            mean_val = non_null.mean() if len(non_null) > 0 else float('nan')
            null_rate = (len(X_train) - len(non_null)) / len(X_train) * 100
            print(f"  {col}: 平均={mean_val:.4f}, 欠損率={null_rate:.1f}%")
