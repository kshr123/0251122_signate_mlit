"""
exp007_tag_features training script

Based on exp006 with the following changes:
1. Add building_tag_id SVD features (90 tags → 15 dims)
2. Add unit_tag_id SVD features (117 tags → 30 dims)

Target: CV MAPE < 14.0% (exp006: 14.19%, improvement > 0.2pt)
"""
...
def train_exp007():
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        for i, idx in enumerate(sorted_indices[:20]):
            ...
            # Mark new features
            if feat_name.startswith('building_tag_svd_') or feat_name.startswith('unit_tag_svd_'):
                marker = "★"
        ...
        # Display new feature importance (tag SVD)
        print("\n  New feature importance (exp007 - tag SVD):")
        tag_features = [f for f in ALL_FEATURES if
                        f.startswith('building_tag_svd_') or f.startswith('unit_tag_svd_')]
        for feat in tag_features[:10]:  # Top 10 tag features only
        ...
        print("=" * 60)
        print(f"  Baseline (exp006): 14.19%")
        print(f"  This result:       {np.mean(cv_scores):.2f}%")
        improvement = 14.19 - np.mean(cv_scores)
        if improvement > 0:
