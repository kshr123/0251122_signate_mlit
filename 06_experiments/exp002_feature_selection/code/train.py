"""
特徴量選択実験スクリプト（exp002_feature_selection）
...
def train_feature_selection():
    ...
    run_name = f"feature_selection_{timestamp}"

    with mlflow.start_run(run_name=run_name):
        print(f"🚀 訓練開始: {run_name}")

        # ===== タグ設定 =====
        mlflow.set_tag("experiment_type", "feature_selection")
        mlflow.set_tag("experiment_id", "exp002")
        ...
        mlflow.log_param("n_drop_flag_columns", len(DROP_FLAG_COLUMNS))
        mlflow.log_param("n_high_missing_columns", len(HIGH_MISSING_COLUMNS))
        ...
        # ===== 前処理（Block System使用） =====
        print("\n🔧 前処理中...")
        print("  - preprocessing.py（Block System）で明示的な前処理を実行")
        print("  - 削除フラグカラム除外")
        print("  - 欠損率95%以上のカラム除外")

        # preprocessing.py の関数を使用
        X_train, X_test, y_train = preprocess_for_training(train, test)
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(cv.split(X_train_np)):
