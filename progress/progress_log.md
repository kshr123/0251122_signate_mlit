...
- **現在のフェーズ**: Phase 3 - 特徴量エンジニアリング準備中
- **最終更新日**: 2025-11-23
- **進捗率**: 約40%（EDA完了、特徴量設計フェーズへ移行）
...
1. **特徴量エンジニアリング**
   - 仕様書作成（01_specs/features.md）
   - **優先実装特徴量**:
     - target_ym分解（年・月・季節フラグ）
     - 都道府県×年月の集約特徴量
     - 築年数（year_built → 連続値化）
     - 位置情報特徴量（緯度経度からの距離等）
   - 欠損値処理戦略の決定
   - カテゴリ変数のエンコーディング（Target Encoding優先）
